...
params:
  ...
  cno: "comp2"
  title: "SA609"
  ## Added series and version on Sep 29, 2020, so I can apply this to different series and versions
  series: "SA609"
  version: "v5"
  sample1: SA609X7XB03505
  clone1: R
  clonelabel1: "X7 UTTTT R"
  sample2: SA609X7XB03554
  clone2: C_H
  clonelabel2: "X7 UUUUU C,H"
  
  
  # For SA906a use different directories
  #sce_dir: "../../data/scrna2/"
  #ca_dir: "../../results/new-cut-v1/outputs/align_clones/clonealign_fit/"
  #clone_dir: "../../results/new-cut-v1/outputs/parse_cnv/gene_clone_cn/"  
...
#                         })
## 20 Oct 2020: changing this to take the median instead of throwing out all the values that are duplicated -- not sure why they are duplicated
tt_df$cnchange <- sapply(tt_df$ensembl_gene_id, 
                         function(x) {
                           if (nrow(df_cnv[df_cnv$ensembl_gene_id==x & df_cnv$cluster==clonelabel1,"median_cnmode"]) == 0) { return(NA) }
                           if (nrow(df_cnv[df_cnv$ensembl_gene_id==x & df_cnv$cluster==clonelabel2,"median_cnmode"]) == 0) { return(NA) }
                           return(as.numeric(
                             median(pull(df_cnv[df_cnv$ensembl_gene_id==x & df_cnv$cluster==clonelabel1,],median_cnmode)) -
                             median(pull(df_cnv[df_cnv$ensembl_gene_id==x & df_cnv$cluster==clonelabel2,],median_cnmode))))
                         })
