...
run_alleleSC_v2 <- function(allele_counts_fn, total_bincnv_fn, output_fn, prefix='ascn'){
  ...
  if(file.exists(allele_counts_fn) & file.exists(total_bincnv_fn)){
    ...
    print("Files dont exist, double check input data!!!")
    return(FALSE)
  }
...
visualize_hm_each_clone <- function(CNbins, clones_labels_df, save_dir, prefix='total'){
  ...
  CNbins <- CNbins %>%
    ...
    ungroup() %>%
    mutate(cell_id = paste0("Clone_", clone_id))
  ...
  for (cl in unique(CNbins$cell_id)){
    cat('###', cl,' \n')
    c <- c + 1
    plots[[cl]] <- plotCNprofileBAF(CNbins, cellid = cl)  #schnapps::
    cat(' \n \n')
  }
  ...
  for(i in rep(1:length(plots))){
    png(paste0(save_dir,prefix,"_",names(plots)[i],".png"), height = 2*300, width=2*800,res = 2*72)
    print(plots[[names(plots)[i]]])
    dev.off()
  }
...
visualize_hm_tree_v2 <- function(ascn_fn, cell_clones_fn, output_fn,
  ...
  png(paste0(save_dir,prefix,"_state_min_ascn.png"), height = 2*800, width=2*1400,res = 2*72)
  print(p2)
  dev.off()
  png(paste0(save_dir,prefix,"_state_BAF_ascn.png"), height = 2*800, width=2*1400,res = 2*72)
  print(p3)
  dev.off()
  png(paste0(save_dir,prefix,"_state_phase_ascn.png"), height = 2*800, width=2*1400,res = 2*72)
  print(p4)
  dev.off()
...
#   
#   if (BAFcol == "state_min"){
#     BAFcolpal <- schnapps::scCNminorallele_cols()
#   }
#   
#   if (BAFcol == "state_phase"){
#     BAFcolpal <- schnapps::scCNphase_cols()
#   }
#   
#   if (BAFcol == "state_AS"){
#     BAFcolpal <- schnapps::scCNAS_cols()
#   }
#   
