...
get_reads_per_clone <- function(input_dir, cellclones_fn, output_file, 
                                obs_clones=NULL,cores_use=NULL){
  ...
  if(is.null(obs_clones)){
    ...
    obs_clones <- unique(cell_clones$clone_id)
    tag <- paste(obs_clones, sep='_')
  }else{  # ex: format: 'B_D' obs_clones <- 'B_D'
    tag <- obs_clones
    obs_clones <- as.character(unlist(strsplit(obs_clones, '_')))
  }
  print(obs_clones)
  ...
  for(c in obs_clones){
    ...
    print(dim(gene_cn))
    data.table::fwrite(gene_cn,file=paste0(output_dir,'mapped_ens_gene_clone_',c,'.csv.gz'))
    mapped_clones <- dplyr::bind_rows(mapped_clones, gene_cn)
    gene_cn2 <- gene_cn %>%
      dplyr::group_by(ensembl_gene_id) %>%
      dplyr::summarise(avg_mappability=mean(avg_mappability),cn_median=mean(cn_median),pct_pure=mean(pct_pure)) %>%
    ...
    reference_genes_df <- reference_genes_df %>% full_join(gene_cn2, by='ensembl_gene_id')
  }  
