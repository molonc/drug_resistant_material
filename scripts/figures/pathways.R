...
if (what == "all_pathways_by_clone_scran") {
  ...
  passages <- c("X4", "X4", "X5", "X5", "X5", "X6", "X6", "X7", "X7", 
              #"Pt5\nX6", "Pt5\nX7", "Pt5\nX7", 
              "X8", "X8", "X10", "X10", "X10",
              "X5", "X6",  "X6", "X7", "X7", "X7", "X7",  "X8", "X8", "X8", "X8"
  )
...
ggsave(filename, width=7.1, heigh=4.7, useDingbats=FALSE)
# 
# ### Plot for Fig 1
...
ct <- NULL
ct <- prepare_cis_trans(cnv_df=Pt4_cnv, series="Pt4 X4", data_num=1, clone1="A", clone2="H", type="Rx")
ct <- prepare_cis_trans(cnv_df=Pt4_cnv, series="Pt4 X4", data_num=2, clone1="B", clone2="H", type="Rx")
ct <- prepare_cis_trans(cnv_df=Pt4_cnv, series="Pt4 X5", data_num=3, clone1="A", clone2="H", type="Rx")
ct <- prepare_cis_trans(cnv_df=Pt4_cnv, series="Pt4 X5", data_num=4, clone1="A", clone2="H", type="RxH")
ct <- prepare_cis_trans(cnv_df=Pt4_cnv, series="Pt4 X5", data_num=5, clone1="B", clone2="H", type="RxH")
...
ct <- prepare_cis_trans(cnv_df=Pt5_cnv, series="Pt5 X9+1", data_num=12, clone1="A", clone2="G", type="RxH")
ct <- prepare_cis_trans(cnv_df=Pt5_cnv, series="Pt5 X9+1", data_num=13, clone1="A", clone2="G", type="Rx")
...
ct <- prepare_cis_trans(cnv_df=Pt6_cnv, series="Pt6 X7H", data_num=20, clone1="H", clone2="E", type="Rx")
ct <- prepare_cis_trans(cnv_df=Pt6_cnv, series="Pt6 X7H", data_num=21, clone1="H", clone2="E", type="RxH")
ct <- prepare_cis_trans(cnv_df=Pt6_cnv, series="Pt6 X8G", data_num=22, clone1="G", clone2="E", type="Rx")
ct <- prepare_cis_trans(cnv_df=Pt6_cnv, series="Pt6 X8G", data_num=23, clone1="G", clone2="E", type="RxH")
...
ggsave("Fig5_boxplots.pdf", width=5.8, heigh=2.1, useDingbats=FALSE)
...
if (clone=="aware") {
  ...
  df <- NULL
  df <- calc_perc_reverted (df, series="Pt4", title="Pt4 X5 HAA", tr="X5 Rx:A / UnRx:H", hol="X5 RxH:A / UnRx:H")
  df <- calc_perc_reverted (df, series="Pt4", title="Pt4 X5 HAB", tr="X5 Rx:A / UnRx:H", hol="X5 RxH:B / UnRx:H")
  df <- calc_perc_reverted (df, series="Pt4", title="Pt4 X6 HAA", tr="X6 Rx:A / UnRx:H", hol="X6 RxH:A / UnRx:H")
  df <- calc_perc_reverted (df, series="Pt4", title="Pt4 X7 HAA", tr="X7 Rx:A / UnRx:H", hol="X7 RxH:A / UnRx:H")
  
  #df <- calc_perc_reverted (df, series="Pt5", title="Pt5 X7", tr="X7 Rx:AEJ vs. UnRx:G", hol="X7 RxH:AEJ vs. UnRx:G")
  df <- calc_perc_reverted (df, series="Pt5", title="Pt5 X8 GAD", tr="X8 Rx:A / UnRx:G", hol="X8 RxH:D / UnRx:G")
  #df <- calc_perc_reverted (df, series="SA535", title="SA535 X9", tr="SA535 Rx X9 vs. UnRx X9", hol="SA535 RxH X9 vs. UnRx X9")
  df <- calc_perc_reverted (df, series="Pt5", title="Pt5 X9+1 GAA", tr="X10 Rx:A / UnRx:G", hol="X10 RxH:A / UnRx:G")
  df <- calc_perc_reverted (df, series="Pt5", title="Pt5 X9+1 GAE", tr="X10 Rx:A / UnRx:G", hol="X10 RxH:E / UnRx:G")
  
  df <- calc_perc_reverted (df, series="Pt6", title="Pt6 X6 GGB", tr="X6 Rx:G / UnRx:G", hol="X6 RxH:B / UnRx:G")
  df <- calc_perc_reverted (df, series="Pt6", title="Pt6 X7 EHH", tr="X7 Rx:H / UnRx:E", hol="X7 RxH:H / UnRx:E")
  df <- calc_perc_reverted (df, series="Pt6", title="Pt6 X7 EGG", tr="X7 Rx:G / UnRx:E", hol="X7 RxH:G / UnRx:E")
  df <- calc_perc_reverted (df, series="Pt6", title="Pt6 X8 EHH", tr="X8 Rx:H / UnRx:E", hol="X8 RxH:H / UnRx:E")
  df <- calc_perc_reverted (df, series="Pt6", title="Pt6 X8 EGG", tr="X8 Rx:G / UnRx:E", hol="X8 RxH:G / UnRx:E")
...
ggsave(paste0("reversed_pathways_clone", clone, ".pdf"), width=6, heigh=2, useDingbats=FALSE)
...
if (clone=="aware") {
  num <- NULL
  num <- calc_num_pathways (num, series="Pt4 X4", title="Pt4 X4 HA", comp="X4 Rx:A / UnRx:H", type="Rx")
  num <- calc_num_pathways (num, series="Pt4 X4", title="Pt4 X4 HB", comp="X4 Rx:B / UnRx:H", type="Rx")
  num <- calc_num_pathways (num, series="Pt4 X5", title="Pt4 X5 HA", comp="X5 Rx:A / UnRx:H", type="Rx")
  num <- calc_num_pathways (num, series="Pt4 X5", title="Pt4 X5 HA", comp="X5 RxH:A / UnRx:H", type="RxH")
  num <- calc_num_pathways (num, series="Pt4 X5", title="Pt4 X5 HB", comp="X5 RxH:B / UnRx:H", type="RxH")
  ...
  num <- calc_num_pathways (num, series="Pt6 X7", title="Pt6 X7 EH", comp="X7 Rx:H / UnRx:E", type="Rx")
  num <- calc_num_pathways (num, series="Pt6 X7", title="Pt6 X7 EH", comp="X7 RxH:H / UnRx:E", type="RxH")
  num <- calc_num_pathways (num, series="Pt6 X7", title="Pt6 X7 EG", comp="X7 Rx:G / UnRx:E", type="Rx")
  num <- calc_num_pathways (num, series="Pt6 X7", title="Pt6 X7 EG", comp="X7 RxH:G / UnRx:E", type="RxH")
  num <- calc_num_pathways (num, series="Pt6 X8", title="Pt6 X8 EH", comp="X8 Rx:H / UnRx:E", type="Rx")
  num <- calc_num_pathways (num, series="Pt6 X8", title="Pt6 X8 EH", comp="X8 RxH:H / UnRx:E", type="RxH")
  num <- calc_num_pathways (num, series="Pt6 X8", title="Pt6 X8 EG", comp="X8 Rx:G / UnRx:E", type="Rx")
  num <- calc_num_pathways (num, series="Pt6 X8", title="Pt6 X8 EG", comp="X8 RxH:G / UnRx:E", type='RxH')
...
ggsave(paste0("number_pathways_clone", clone, ".pdf"), width=5.75, height=2.1, useDingbats=FALSE)
