...
# output_dir <- paste0(save_dir,'rstmm4_SA609_UTTTT_R_UUUUU_H/')
# de_genes_609 <- data.table::fread(paste0(output_dir,'signif_genes_FDR0.01.csv')) %>% as.data.frame()

de_genes_609 <- data.table::fread(paste0(output_dir,datatag,'/SA609_4_SA609_UTTTT_R_UUUUU_H/signif_genes_FDR_0.01.csv')) %>% as.data.frame()
...
vol_609 <- plot_DE_genes_ggplot(de_genes_609, get_top_genes(de_genes_609), capstr='', 
                                FDRcutoff=0.01, logFCcutoff=0.25, pValuecutoff=0.05,
                                plttitle, save_dir=output_dir, legendVisible=F,
                                iscaption=TRUE, legend_verbose='none', save_plot=F,
                                xl=c(-3.5,3.5), yl=c(0, 320))

vol_609_legend <- plot_DE_genes_ggplot(de_genes_609, get_top_genes(de_genes_609), capstr='', FDRcutoff=0.01, logFCcutoff=0.25, pValuecutoff=0.05,
                                       plttitle, save_dir=output_dir, legendVisible=F,
...
plg_cis <- cowplot::ggdraw() + cowplot::draw_plot(cowplot::get_legend(vol_609_legend$p_vol_cis +  guides(color = guide_legend(nrow = 2, title = NULL))))
plg_trans <- cowplot::ggdraw() + cowplot::draw_plot(cowplot::get_legend(vol_609_legend$p_vol_trans +  guides(color = guide_legend(nrow = 1, title = NULL))))
vol_609$p_vol_cis
vol_609$p_vol_trans
vol_609$p_prop_cis
vol_609$p_prop_trans
volplotls <- cowplot::plot_grid(vol_609$p_vol_cis, vol_609$p_vol_trans,
                                ncol=1, align = 'v')
volplot_prop <- cowplot::plot_grid(vol_609$p_prop_cis, plg_cis, 
                                vol_609$p_prop_trans, plg_trans, 
                                ncol=1, align = 'v', 
...
ptotal_track_vol <- cowplot::plot_grid(track_609, ptotal, rel_heights = c(1,1.2), ncol=1)

png(paste0(save_fig_dir,"Fig4_volcanos_track_SA609.png"), height = 2*900, width=2*650, res = 2*72)
print(ptotal_track_vol)
dev.off()

ggsave(paste0(save_fig_dir,"Fig4_volcanos_track_SA609.pdf"),
...
ggsave(paste0(save_fig_dir,"Fig4_volcanos_track_SA609.png"),
       plot = ptotal_track_vol,
       height = 9,
       width = 7.5,
       # useDingbats=F,
