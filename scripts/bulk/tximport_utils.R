...
load_raw_counts_kallisto <- function(sample_ids, input_dir, datatag, save_dir){
  ...
  # ref_tx2gene_fn <- '/home/htran/storage/datasets/drug_resistance/rna_results/biodatabase/tx2gene.csv'
  ...
  txi <- tximport(input_fns, type = "kallisto", tx2gene = tx2gene, ignoreAfterBar = TRUE)
  # class(txi$counts)
  head(txi$abundance)
  class(txi$length)
...
load_data <- function(results_dir, input_dir, base_name){
  # source('/home/htran/Projects/farhia_project/rnaseq/bulk/tximport_utils.R')
  samples <- list.files(input_dir)
  print(samples)
  meta_df <- data.table::fread(paste0(results_dir,'bulk_metadata_v2.csv'), header=T) %>% as.data.frame()
  if(base_name=='SA609'){
    samples <- samples[grepl('3',samples)] #  
    meta_df <- meta_df %>%
      dplyr::filter(datatag==base_name)
  }
  
  if(base_name=='SA535'){
    ts <- unique(meta_df$treatmentSt)
    ts <- ts[!grepl('CX5461',unique(meta_df$treatmentSt))]
    meta_df <- meta_df %>%
      dplyr::filter(datatag==base_name & treatmentSt %in% ts)
    meta_df$treatmentSt <- gsub('^Cisplatin:','',meta_df$treatmentSt)
  }  
  print(dim(meta_df))
  rownames(meta_df) <- meta_df$bulk_sample
  samples <- samples[samples %in% meta_df$bulk_sample]
  
  counts_df <- load_counts_data(samples, input_dir, base_name)
  counts_df$gene_symb <- rownames(counts_df)
  data.table::fwrite(counts_df, paste0(results_dir,'bulk_counts_total_samples.csv.gz'))
  print(head(counts_df))
  
  counts_df$gene_symb <- NULL
  
  lcpm_dge <- load_dge_counts(as.matrix(counts_df), meta_df[samples,'treatmentSt'])
  lcpm_dge <- as.data.frame(lcpm_dge)
  lcpm_dge$gene_symb <- rownames(lcpm_dge)
  data.table::fwrite(lcpm_dge, paste0(results_dir,'bulk_CPM_total_samples.csv.gz'))
  print(head(lcpm_dge))
  lcpm_dge <- lcpm_dge %>% 
    tidyr::pivot_longer(!gene_symb, names_to = 'bulk_sample', values_to = 'expr')
  lcpm_dge$bulk_sample <- as.character(lcpm_dge$bulk_sample)
  # dim(lcpm_dge2)
  # View(head(lcpm_dge))
  # View(meta_df1)
  # dim(lcpm_dge)
  meta_df$bulk_sample <- as.character(meta_df$bulk_sample)
  lcpm_dge <- lcpm_dge %>% inner_join(meta_df, by=c('bulk_sample'))
  data.table::fwrite(lcpm_dge, paste0(results_dir,'bulk_CPM_total_samples_metadata.csv.gz'))
...
get_ens_gene_ids <- function(gene_ids){
  genes <- mclapply(strsplit(gene_ids,'\\.'), function(x){
    return(x[1])
  }, mc.cores=3)
  return(as.character(genes))
}

load_dge_counts <- function(cts, save_dir, datatag, groups_use=NULL){
  # Creating a DGEList object for use in edgeR.
  # class(cts)
  # dim(cts)
  # head(cts)
  ...
  keep <- edgeR::filterByExpr(dge)
  # dge <- dge[keep, ]
  #It is recommended to recalculate the library sizes of the DGEList object
  ...
  saveRDS(dge, paste0(save_dir, datatag, '_dge_preprocessed.rds'))
  lcpm_dge <- edgeR::cpm(dge, log=TRUE)
  print(dim(lcpm_dge))
  lcpm_dge <- as.data.frame(lcpm_dge)
  lcpm_dge$ens_gene_id <- rownames(lcpm_dge)
  data.table::fwrite(lcpm_dge, paste0(save_dir, datatag, 'bulk_total_samples_normalized_CPM.csv.gz'))
...
get_meta_data <- function(){
  meta_df <- data.table::fread(paste0(results_dir,'bulk_metadata.csv'), header=T) %>% as.data.frame()
  # View(meta_df)
  meta_df$datatag <- stringr::str_sub(meta_df$treatmentSt,1,5)
  rownames(meta_df) <- meta_df$sample
  meta_df$bulk_sample <- ''
  for(s in meta_df$sample){
    meta_df[s,'bulk_sample']  <- stringr::str_sub(s,nchar(s)-4,nchar(s))
  }
  meta_df$bulk_sample <- NULL
  meta_df <- meta_df %>%
    dplyr::rename(timepoint=time)%>%
    dplyr::mutate(bulk_sample=stringr::str_sub(sample,nchar(sample)-3,nchar(sample)))
  
  length(samples)
  meta_df$V5 <- NULL
  meta_df$treatmentSt <- gsub('SA609:|SA535:','',meta_df$treatmentSt)
  data.table::fwrite(meta_df, paste0(results_dir,'bulk_metadata_v2.csv'))
  
  # meta_df$datatag <- ifelse(meta_df$datatag=='UnRx','SA535',meta_df$datatag)
  
}

viz_lineplot_gene_expr <- function(df, obs_gene=''){
  
  cols_use <- c('darkgrey','blue','#CCCC00')
  names(cols_use) <- c('UnRx','Rx','RxH')
  dh_times <- df %>%
    dplyr::filter(treatmentSt=='RxH') %>%
    dplyr::pull(timepoint)
  dh_times <- gtools::mixedsort(dh_times)
  dh_times <- dh_times[1] # get earliest DH time point
  
  tmp <- df %>% 
    dplyr::filter(treatmentSt=='Rx' & timepoint==paste0('X',as.numeric(gsub('X','',dh_times))-1))
  tmp$treatmentSt <- 'RxH'
  df <- dplyr::bind_rows(tmp, df)
  df$timepoint <- factor(df$timepoint, levels=gtools::mixedsort(unique(df$timepoint)))
  p <- ggplot(df, aes(x=timepoint, y=expr, group=treatmentSt)) +
    geom_line(aes(color=treatmentSt))+ #, linetype=treatmentSt
    geom_point(aes(color=treatmentSt), size=4) + 
    scale_color_manual(values=cols_use, name='Treatment') + 
    scale_y_continuous(breaks = scales::pretty_breaks(n = 5)) + 
    theme_bw() + 
    ylim(min(df$expr)-0.05,max(df$expr)+0.05) + 
    labs(x='Passage',y='Gene expr (logCPM)',title=paste0('Bulk Gene: ',obs_gene))
  return(p)
}
...
create_tx2gene_table <- function(){
  # BiocManager::install('tximportData')
  library(tximportData)
  dir <- system.file("extdata", package = "tximportData")
  list.files(dir)
  tx2gene_ens_v3 <- read.table(file.path(dir, "tx2gene.gencode.v27.csv"), header = TRUE)
  dim(tx2gene_ens_v3)
  tx2gene_ens_v3$TXNAME <- get_gene_ids(tx2gene_ens_v3$TXNAME.GENEID, 1)
  tx2gene_ens_v3$GENEID <- get_gene_ids(tx2gene_ens_v3$TXNAME.GENEID, 2)
  tx2gene_ens_v3 <- tx2gene_ens_v3 %>%
    dplyr::select(TXNAME, GENEID)
  data.table::fwrite(tx2gene_ens_v3, '/home/htran/storage/datasets/drug_resistance/rna_results/biodatabase/tx2gene_updated_06_Dec_2021.csv.gz')
  return(tx2gene_ens_v3)
}


get_gene_ids <- function(gene_ids, idx=1){ # idx: 1 transcript, 2: ens gene ids
...
# gids <- c('TMEM200B|ENSG00000253304.1', 'NIT1|ENSG00000158793.9')
# get_ens_gene_from_gene_index <- function(gids){
#   genes <- mclapply(strsplit(gids,'\\|'), function(x){
#     return(x[2])
#   }, mc.cores =3)
#   return(as.character(genes))
# }
# 
# 
# ens_gene_ids <- get_ens_gene_from_gene_index(gids)
# 
# get_ens_gene_without_version <- function(ens_gene_ids){
#   genes <- mclapply(strsplit(ens_gene_ids,'\\.'), function(x){  #mclapply --> lapply is ok
#     return(x[1])
#   }, mc.cores =3)
#   return(as.character(genes))
# }
# get_ens_gene_without_version(ens_gene_ids)
# 
# get_gene_symbol_from_gene_index <- function(gids){
#   genes <- mclapply(strsplit(gids,'\\|'), function(x){
#     return(x[1])
#   }, mc.cores =3)
#   return(as.character(genes))
# }
# get_gene_symbol_from_gene_index(gids)
