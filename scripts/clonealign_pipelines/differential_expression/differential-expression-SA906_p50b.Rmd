...
author:
  - "Kieran Campbell"
params:
  rmd: "differential-expression-SA906_p50b.Rmd"
  sample: SA609
  output_fig_dir: home
  input_sce: sce
  input_ca: ca
...
```{r}
dplyr::filter(df_tsne, Clone %in% c("B", "D", "E")) %>% 
...
# Differential expression D vs E

```{r}
sce_de <- sce_qc[, sce_qc$clone %in% c("D", "E")]
...
```{r}
saveRDS(tt, glue("{params$output_fig_dir}/{params$sample}_differential_expression_d_e.rds"))
```
...
ggplot(tt, aes(x = logFC, y = -log10(FDR))) +
  ...
  labs(x = "log FC, clone E vs clone D")
```
...
```{r}
saveRDS(last_plot(), glue("{params$output_fig_dir}/{params$sample}_gex_cnv_plot_d_vs_e.rds"))
```


# Differential expression D vs B

```{r}
sce_de <- sce_qc[, sce_qc$clone %in% c("D", "B")]
...
```{r}
saveRDS(tt, glue("{params$output_fig_dir}/{params$sample}_differential_expression_d_b.rds"))
```
...
ggplot(tt, aes(x = logFC, y = -log10(FDR))) +
  geom_point() +
  geom_text_repel(data = df_annot, aes(label = gene_symbol)) +
  labs(x = "log FC, clone E vs clone B")
```
...
```{r}
saveRDS(last_plot(), glue("{params$output_fig_dir}/{params$sample}_gex_cnv_plot_d_vs_b.rds"))
```
