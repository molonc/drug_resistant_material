...
viz_genes_exp_lineages_cistrans_anno_hm <- function(exp_mtx, obs_genes_df, obs_cells_df, 
  ...
  gt <- c('in cis','in trans')
  col_gt <- c("in cis" = "darkorange", "in trans" = "darkcyan")
  ...
  anno_row <- ComplexHeatmap::rowAnnotation(
    GeneType = as.matrix(cistrans_anno_df), #gap = unit(2, "points"),
    col = list(GeneType = col_gt), simple_anno_size = unit(0.5, "cm"), show_legend = F
  )
...
viz_given_gene_exp_lineages <- function(obs_genes_ls, meta_genes, avg_gene_exp, output_dir, datatag){
  ...
  for(i in seq(obs_genes_ls)){
    ...
    if(!is.null(obs_gene)){
      ...
      # verbose_lg
      p <- viz_smooth_genes_cluster_v2(gsymb, sigf_gene_exp, 
                                  obs_gene, paste0(gsymb, ': ',names(gsymb)), output_dir,
      ...
      p1 <- viz_smooth_genes_cluster_v2(gsymb, sigf_gene_exp, 
                                       obs_gene, paste0(gsymb, ': ',names(gsymb)), output_dir,
                                       verbose_legend = 'none')
...
viz_smooth_genes_cluster_v2 <- function(gene_module, sigf_gene_exp, g, gsymb, 
  ...
  for(g in unique(sigf_gene_exp$gene)){
    ...
    p <- p + geom_line(data = gene_exp,
                       aes(x=time, y=yhat, col = lineage_desc, group = lineage_desc), lwd = 1)
  }
